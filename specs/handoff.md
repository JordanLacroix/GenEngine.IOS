...
- **Observé au simulateur, sur un seul appareil.** L'accueil anonyme, la scène d'introduction, le menu, la coque HUD, l'accueil de démonstration et une partie de démonstration ont été **regardés tourner** sur iPhone 17 Pro / iOS 26.5, contre la pile locale. Trois défauts de rendu bloquants y ont été trouvés et corrigés (voir ci-dessous). **iPad, l'écran de paramètres, la carte, la bibliothèque, le Studio et l'administration n'ont toujours pas été observés**, non plus qu'aucun rendu sur appareil physique ni en Dynamic Type agrandi.
...
- **Valeurs de `HUDMetrics` toujours non calibrées.** `topBarHeight: 74`, `bottomBarHeight: 96`, `railWidth: 108` restent des estimations. Une fois la barre basse contrainte à la hauteur de son contenu, elle mesure environ 79 pt sur iPhone 17 Pro : `bottomBarHeight: 96` sur-réserve donc un peu, sans rien recouvrir. `railWidth` n'a pas été observé, faute de rendu iPad. Seul `minimumTarget: 44` correspond à une contrainte réelle.
- **Bandeau interne toujours pas vu à l'écran.** La seconde barre d'onglets a été retirée et le bandeau restant s'appuie sur `HUDMetrics` ; l'écran « Mon univers » n'a pas été ouvert, faute de compte de test joué jusque-là. Cette décision reste correcte par construction, pas par constat.
...
La priorité reste de **regarder l'application tourner**, mais sur ce qui ne l'a pas encore été : l'écran de paramètres, la bibliothèque, la carte du joueur, le Studio et l'administration, en simulateur **iPad** (le rail vertical n'a jamais été rendu) et en Dynamic Type agrandi. La première passe iPhone a suffi à trouver trois défauts bloquants que build et tests laissaient passer ; il n'y a pas de raison de croire les écrans restants indemnes.

Calibrer `HUDMetrics` en découle toujours, `railWidth` en premier.
