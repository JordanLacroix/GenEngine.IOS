...
- **Catalogue paginé, recherche serveur.** `GET /catalog` est consommé sous la forme de l'enveloppe unifiée `{ items, page, pageSize, total }` de GenEngine#55, via `PagedList` (`Core/Models/PagedList.swift`), dont le décodage est tolérant : un tableau nu — le contrat antérieur — reste lu comme une page unique complète, et une métadonnée absente retombe sur une valeur cohérente avec `items`. `AppState` conserve `catalogPage`, `catalogTotal` et `catalogQuery` ; `loadMorePublishedStories()` ajoute la page suivante et `searchCatalog(_:)` repart de la première page avec une recherche **appliquée par le serveur**, jamais par filtrage de la page affichée. `units` et `periods` sont parcourus page par page dans `LiveGenEngineAPI.allPages`, l'écran d'administration ayant réellement besoin de l'arbre entier ; le journal envoie `pageSize` et non plus `limit`.

- **Plus aucune liste plafonnée en silence.** Cinq listes ne demandaient qu'une page sans jamais l'annoncer : journal (100), utilisateurs (50), appartenances (100), affectations (100) et scénarios du Studio (50). Au-delà du plafond, les éléments manquants disparaissaient **sans aucun signal** — un écran incomplet se lisant exactement comme un écran complet. Aucun de ces cinq écrans n'offrant de défilement paginé ni d'indicateur de reste, tous passent désormais par `allPages` et reçoivent la collection entière ; le `total` rendu est l'effectif réellement chargé, si bien que `adminUsersTotal` et `authorScenariosTotal` cessent d'afficher la taille d'une page. Le plafond de sécurité de 100 pages (10 000 éléments) subsiste mais **n'est plus silencieux** : l'atteindre lève `APIError.incompleteList`, que l'appelant présente comme n'importe quel autre refus. Le catalogue, lui, est inchangé : il est réellement paginé dans l'interface. Couvert par `AuthoringRouteTests`.

- **La sauvegarde de brouillon vise la bonne route.** Le Studio appelait `PUT /scenarios/{id}`, que le moteur n'expose pas : vérifié sur la pile locale, cette route répond **405** quand `PUT /scenarios/{id}/draft` répond 401. Chaque enregistrement de brouillon échouait donc. Le corps `{ expectedRevision, document }` était déjà le bon ; seul le chemin manquait le suffixe `/draft`, comme le fait correctement le client web.

  Comportement par écran : **Bibliothèque**, surface exhaustive, chargement progressif au défilement plus un bouton « Charger la suite » et un compteur « n sur total » ; **Accueil**, même chargement progressif au défilement ; **carte du joueur**, dock horizontal `LazyHStack` qui demande la page suivante en fin de défilement et avance dans les pages tant que la porte sélectionnée n'a encore aucun récit chargé.

  **Deux paginations distinctes coexistent sur la carte et ne doivent pas être confondues.** `doorPage` pagine l'**affichage** des portes quand le viewport ne peut pas toutes les porter : c'est un état local à la vue, sans aucun appel réseau, portant sur les catégories du document de configuration. La pagination du catalogue porte sur le **contenu** servi par `GET /catalog`. L'avance automatique du dock est donc clavetée sur `selectedCategoryID`, jamais sur `doorPage` : changer de page de portes ne déclenche aucun chargement, et une porte n'est pas une page de catalogue.
...
- **Quatre doublons de l'enveloppe paginée subsistent dans `APIModels.swift`.** `PagedMembershipsView`, `PagedAssignmentsView`, `PagedScenariosView` et `PagedUsersView` redéclarent à la main `{ items, page, pageSize, total }` avec le décodage synthétisé, au lieu de passer par `PagedList`. Ils ne bénéficient donc pas de sa tolérance : face à un serveur antérieur à GenEngine#55 qui répond par un **tableau nu**, ou à une réponse dont une métadonnée manque, le décodage échoue entièrement là où `PagedList` aurait lu une page. Le parcours de pages introduit par cette PR décode déjà via `PagedList` et n'est donc pas exposé, mais les quatre types le restent pour tout autre appel. **Correctif à faire : les remplacer par des alias `PagedList`**, comme `PagedUnitsView` et consorts. Non traité ici pour ne pas entrer en conflit avec un travail parallèle sur `APIModels.swift`.
