...
struct AuthoringRouteTests {
    ...
    @Test func scenariosWalkEveryPage() async throws {
        ...
        let pages = recorder.calls.compactMap { $0.queryValue("page") }
        #expect(pages == ["1", "2", "3"])
    }

    @Test func usersWalkEveryPage() async throws {
        RouteStub.install { request in (200, Fixtures.page(of: 130, at: request.url, key: "user")) }
        defer { RouteStub.reset() }
    ...
    @Test func userSearchIsForwardedOnEveryPage() async throws {
        ...
        _ = try await makeAPI().users(query: "dupont")
        let calls = recorder.calls
        #expect(calls.count == 2)
        #expect(calls.allSatisfy { $0.queryValue("query") == "dupont" })
    }

    @Test func membershipsWalkEveryPage() async throws {
        RouteStub.install { request in (200, Fixtures.page(of: 250, at: request.url, key: "membership")) }
        defer { RouteStub.reset() }
    ...
    @Test func assignmentsWalkEveryPage() async throws {
        RouteStub.install { request in (200, Fixtures.page(of: 250, at: request.url, key: "assignment")) }
        defer { RouteStub.reset() }
    ...
    @Test func journalWalksEveryPageAndKeepsServerTotals() async throws {
        RouteStub.install { request in (200, Fixtures.page(of: 340, at: request.url, key: "journal")) }
        defer { RouteStub.reset() }

        let journal = try await makeAPI().journal(frontId: "default")
        #expect(journal.items.count == 340)
        #expect(journal.total == 340)
        // `totalsByType` est un agrégat sur tout le journal, pris sur la première page.
        #expect(journal.totalsByType["ScenarioCompleted"] == 340)
    }
    ...
    @Test func exceedingThePageCeilingIsAnnouncedRatherThanTruncated() async throws {
        RouteStub.install { request in (200, Fixtures.page(of: 20_000, at: request.url, key: "scenario")) }
        defer { RouteStub.reset() }
    ...
    @Test func theCeilingErrorNamesWhatIsMissing() async throws {
        ...
        do {
            ...
            Issue.record("Une liste tronquée doit lever une erreur.")
        } catch let error as APIError {
            guard case let .incompleteList(loaded, total) = error else {
                Issue.record("Erreur inattendue : \(error)")
                return
            }
            #expect(loaded == 10_000)
            #expect(total == 20_000)
            let message = try #require(error.errorDescription)
            #expect(message.contains("10000") || message.contains("10 000"))
        }
    }
    ...
    @Test func aBareArrayIsReadAsOneCompletePage() async throws {
        ...
        RouteStub.install { request in
            recorder.record(request)
            return (200, Fixtures.bareScenarioArray(count: 4))
        }
        defer { RouteStub.reset() }
